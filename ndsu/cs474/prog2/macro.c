...
MACRO *in;
{
    ...
    {
	for (; i < 3; i++)
	    temp->Data[0] = (temp->Data[0] * 8) + (nxtchar(in) -'0');
	temp->Data[1] = EOF;
    }
}
...
MACRO *in;
{
	...
	{
	case '$':
	    switch ((ch = nxtchar(in)))
	    {
	    case '$':
		WriteChar('$');
		break;
	    case '0':
		if (LocateMacro(ch) == NULL)
		    CreateMacro(in);
		break;
	    case '1':
		while (nxtchar(in) !='\n');
