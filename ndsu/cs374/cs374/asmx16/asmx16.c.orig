...
main(int argc, char *argv[])
{
	...
	char buffer[80]="",                     /* array for input line */
		 cbuffer[80]="";                /* array for input line
						   w/o comment          */
...
----------------------------------------------------------------------------*/
	...
	if (strcmp(tmps, DIR_PROCEDURE)==0)
		procedure_flag=1;
	else if (strcmp(temp1,DIR_EQU)==0)  {
		wptr=(struct symbol_entry *)malloc(sizeof(struct symbol_entry));
		add_symbol_entry(wptr, tmps, TYPE_NONE, "", get_nbr(temp2), 0);

	}
	else if(strcmp(tmps, DIR_ORG)==0) {
		lc=get_nbr(temp1);
	}

	else if(strcmp(tmps, DIR_PROGRAM))
	...
	for ( ; ; )   {
		...
		else if(strcmp(temp, DIR_ORG)==0) {
			lc=get_nbr(temp1);
		}
		else if (strcmp(temp,DIR_PROCEDURE)==0)  {
			found = 1;
			procedure_flag=1;
		}
		else if (!program_flag)   {
			if (!procedure_flag)
				break;
		}
		...
		else if(strcmp(temp, DIR_ENDP)==0) {
			if(procedure_flag)
				procedure_flag=0;
			/*do nothing*/
		}
		else if (strcmp(temp, DIR_END)==0)  {
			/*goto PASS 2*/
		   break;
		}
		else    {
		...
		strcpy(temp, "");  strcpy(temp1, "");   strcpy(temp2, "");
	}
	fclose(src_file_ptr);
...
----------------------------------------------------------------------------*/
	...
	for ( ; ; )   {
		...
		if ((strcmp(temp1,DIR_DB)==0) || (strcmp(temp1,DIR_DH)==0) ||                                (strcmp(temp1,DIR_DW)==0) || (strcmp(temp1,DIR_DD)==0)){
			...
			else if (!strcmp(temp, DIR_PROGRAM) || !strcmp(temp, DIR_PROCEDURE)){
			}
			else {
		...
		strcpy(buffer,"");  strcpy(temp, ""); strcpy(temp1, "");
		strcpy(temp2, "");
	}
	fclose(src_file_ptr);
...
{	int i,value;
	for (i=1,value=nbr;i<pwr;i++) value = value * nbr;
	return(value);
}
...
{	int i,value;
	if (s[strlen(s)-1]=='H'||s[strlen(s)-1]=='h')		      /*if hex*/
		sscanf(s,"%x",&value);
	else if (s[strlen(s)-1]=='B'||s[strlen(s)-1]=='b')	   /*if binary*/
	{
		if (s[strlen(s)-2]=='1') value = 1;
		else value = 0;
		for (i=1;i<(strlen(s)-1);i++)
		{
			if (s[strlen(s)-2-i]=='1') value += power(2,i);
		}
	} else    sscanf(s,"%d",&value);
	return(value);
}
...
{	if (!strcmp(buf,"R0")) return(REG_R0);
	if (!strcmp(buf,"R1")) return(REG_R1);
	if (!strcmp(buf,"R2")) return(REG_R2);
	if (!strcmp(buf,"R3")) return(REG_R3);
	if (!strcmp(buf,"R4")) return(REG_R4);
	if (!strcmp(buf,"R5")) return(REG_R5);
	if (!strcmp(buf,"SP")) return(REG_SP);
	if (!strcmp(buf,"PC")) return(REG_PC);
	return(REG_ERR);
}
...
------------------------------------------------------------------------------*/
int get_data_size(char *dir, char *s, struct line_info *lptr,int *lcnt)
{
	int	nbr, mult;
	char	c;
	...
	if( s[0] == '(' )
	{
		...
		--------------------------------------------------------------*/
		for (cnt=0,i=1;s[i]!=')';i++) { if (s[i] == ',') cnt++; }
		for (bcnt=0,i=0;i<=cnt;i++)
		{
			...
			} else
			{
				if (strlen(buf)>8)
				{
					...
					buf[strlen(buf)-8] = '\0';
					sscanf(buf,"%d",&tmpl2);
				} else	sscanf(buf,"%d",&tmpl);
			}
			bcnt += mult;
		}
		if (bcnt%2) return(bcnt+1);
		else return(bcnt);
	}
}
...
/*------------------------------------------------------------------------------
	...
	Returns:  number of bytes that are used/needed.
------------------------------------------------------------------------------*/
int get_dsize(char *dir, char *s, struct line_info *lptr, int *lcnt)
{
	int	nbr, mult;
	char	c;
	char	buf[MAX_CHARS_PER_LINE];
		...
		for (bcnt=0,i=0;i<=cnt;i++)
		{
			...
			switch(mult)
			{
				case 1:
					if ((bcnt%8)==0)
						lptr->r |= (tmpl << 8);
					else if ((bcnt%8)==7) lptr->r = tmpl;
					else if ((bcnt%8)==6)
						lptr->oper2 |= (tmpl << 8);
					else if ((bcnt%8)==5) lptr->oper2=tmpl;
					else if ((bcnt%8)==4)
						lptr->oper1 |= (tmpl << 8);
					else if ((bcnt%8)==3) lptr->oper1=tmpl;
					else if ((bcnt%8)==2)
						lptr->opcode |= (tmpl << 8);
					else if ((bcnt%8)==1) lptr->opcode=tmpl;
					break;
				case 2:
					if ((bcnt%8)==0) lptr->r = tmpl;
					else if ((bcnt%8)==6) lptr->oper2=tmpl;
					else if ((bcnt%8)==4) lptr->oper1=tmpl;
					else if ((bcnt%8)==2) lptr->opcode=tmpl;
					break;
				case 4:
					if ((bcnt%8)==0)
					{	lptr->r = (tmpl>>16);
			...
			if ((bcnt%8)>4 || (bcnt%8)==0) lptr->addr=DOUBLE_LINE;
			else if ((bcnt%8)>2) lptr->addr=WORD_LINE;
			else lptr->addr=HALFWORD_LINE;
			if (!(bcnt%8)&&(i<cnt))
			{	lptr++;
				(*lcnt)++;
				lptr->oper1 = BLANK_NBR;
...
{	int	i;
	...
	{	if (oper1str[i] == '[') bracketflag1 = TRUE;
	 	if (oper1str[i] == '+') plusflag1 = TRUE;
		if (oper1str[i] == '-') minusflag1 = TRUE;
	}
	bracketflag2 = FALSE;
	plusflag2 = FALSE;
	minusflag2 = FALSE;
	for (i=0;i<strlen(oper2str);i++)
	{	if (oper2str[i] == '[') bracketflag2 = TRUE;
	 	if (oper2str[i] == '+') plusflag2 = TRUE;
	 	if (oper2str[i] == '-') minusflag2 = TRUE;
	}
	...
	else if (oper1str[0] == '#')				   /*immediate*/
	{
		...
		{ 	for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
			...
			if (wptr!=NULL_PTR) lptr->oper1 = wptr->value;
		}
		switch(opcodelen)
		{	case 4:
				i = MODE_IMM6;
	...
	else if (oper1str[0] == '[' && plusflag1==TRUE)     /*post increment*/
	{	lptr->oper1 = BLANK_NBR;
	...
	else if (oper1str[0] == '-' && oper1str[1] == '[')   /*pre-decrement*/
	{	lptr->oper1 = BLANK_NBR;
	...
	else if (oper1str[0] == '[')			   /*register indirect*/
	{	lptr->oper1 = BLANK_NBR;
		strcpy(tmps,&oper1str[1]);
		strcpy(oper1str,tmps);
		oper1str[strlen(oper1str)-1] = '\0';
	...
	else if (bracketflag1 == TRUE)				    /*relative*/
	{	for (i=0;oper1str[i]!='[';i++);
		...
		{	for (i=0;oper1str[i]!='+'&&oper1str[i]!='-';i++);
			...
			if (wptr!=NULL_PTR)
			{	offset = wptr->value;
				if (wptr->type == TYPE_NONE) lptr->n = 'N';
				else lptr->r = 'R';
			} else {
				/* error */
				error_count++;
			...
			if (isdigit(tmps2[0]))
			{	if (plusflag1 == TRUE) offset += get_nbr(tmps2);
				else offset -= get_nbr(tmps2);
			} else
			{	for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
				      strcmp(tmps2,wptr->name);wptr=wptr->next);
				if (wptr!=NULL_PTR)
				{      if (plusflag1==TRUE) offset+=wptr->value;
				       else offset -= wptr->value;
				       if (wptr->type==TYPE_NONE) lptr->n='N';
				       else lptr->r = 'R';
				} else ;		/* error */
			}
		} else if (isdigit(oper1str[0])) offset = get_nbr(oper1str);
		else
		{	for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
	...
	else if (plusflag1==TRUE || minusflag1==TRUE)
	{	for (i=0;oper1str[i]!='+'&&oper1str[i]!='-';i++);
		oper1str[i] = ' ';
		sscanf(oper1str,"%s%s",tmps,tmps2);
		for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
	...
	else if (isalpha(oper1str[0]))			/*label,string*/
	{
		for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
			strcmp(oper1str,wptr->name);wptr=wptr->next)
			{}
...
/*					error_count++;  */
	...
	else if (isdigit(oper1str[0]))			     /*address,numeric*/
	{	lptr->r = ' ';
	...
	} else if (oper2str[0] == '#')				   /*immediate*/
	{	/* error - invalid addressing mode */
		error_count++;
	}
	else if (oper2str[0] == '[' && plusflag2==TRUE)     /*post increment*/
	{	lptr->oper2 = BLANK_NBR;
	...
	else if (oper2str[0] == '-' && oper2str[1] == '[')   /*pre-decrement*/
	{	lptr->oper2 = BLANK_NBR;
	...
	else if (oper2str[0] == '[')			   /*register indirect*/
	{	lptr->oper2 = BLANK_NBR;
		strcpy(tmps,&oper2str[1]);
		strcpy(oper2str,tmps);
		oper2str[strlen(oper2str)-1] = '\0';
	...
	else if (bracketflag2 == TRUE)			    /*relative*/
	{	for (i=0;oper2str[i]!='[';i++);
		...
		{	for (i=0;oper2str[i]!='+'&&oper2str[i]!='-';i++);
			...
			if (isdigit(tmps2[0]))
			{	if (plusflag2 == TRUE) offset += get_nbr(tmps2);
				else offset -= get_nbr(tmps2);
			} else
			{	for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
					  strcmp(tmps2,wptr->name);wptr=wptr->next);
				if (wptr!=NULL_PTR)
				{      if (plusflag2==TRUE) offset+=wptr->value;
				       else offset -= wptr->value;
				       if (wptr->type==TYPE_NONE) lptr->n='N';
				       else lptr->r = 'R';
				} else {
		...
		} else if (isdigit(oper2str[0])) offset = get_nbr(oper2str);
		else
		{	for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
	...
	} else if (plusflag2==TRUE || minusflag2==TRUE)
	{	for (i=0;oper2str[i]!='+'&&oper2str[i]!='-';i++);
		oper2str[i] = ' ';
		sscanf(oper2str,"%s%s",tmps,tmps2);
		for (wptr=&symbol_anchor;wptr!=NULL_PTR &&
	...
	else if (isalpha(oper2str[0]))			/*label,string*/
	{	/*--------------------------------------------------------------
	...
	else if (isdigit(oper2str[0]))				/*address, numeric */
...
{	int	i,j;
	...
	{	if (larr[j].addr == COMMENT_LINE);
		else if (larr[j].addr == HALFWORD_LINE)
			objarr[++hwcnt] = larr[j].opcode;
		else if (larr[j].addr == WORD_LINE)
		{	objarr[++hwcnt] = larr[j].opcode;
			objarr[++hwcnt] = larr[j].oper1;
		}
		else if (larr[j].addr == DOUBLE_LINE)
		{	objarr[++hwcnt] = larr[j].opcode;
		 	objarr[++hwcnt] = larr[j].oper1;
...
{	int	j;
	...
	{	if ((larr[j].addr==COMMENT_LINE) || (larr[j].addr==ORG_LINE))
		...
		{
			if (larr[j].addr == BLANK_NBR) fprintf(fxlt,"    ");
			else	fprintf(fxlt,"%04X",larr[j].addr);
			if (larr[j].error == NO_ERROR)
				fprintf(fxlt,"   ");
			else
				fprintf(fxlt," %c ",larr[j].error);
			if (larr[j].opcode == BLANK_NBR) fprintf(fxlt,"     ");
			else fprintf(fxlt,"%04X ",larr[j].opcode);
			if (larr[j].oper1 == BLANK_NBR) fprintf(fxlt,"     ");
			else fprintf(fxlt,"%04X ",larr[j].oper1&0x0000FFFF);
			if (larr[j].oper2 == BLANK_NBR) fprintf(fxlt,"     ");
			else fprintf(fxlt,"%04X ",larr[j].oper2&0x0000FFFF);
