...
###############################################################################
{
	...
	my $old_user_name = $_[2];
	my $new_user_name = $_[3];
	my $old_prob_space = $_[4];
	my $new_prob_space = $_[5];
	my $old_event_date = $_[6];
	my $new_event_date = $_[7];
	my $old_duration = $_[8];
	my $new_duration = $_[9];
	my $old_event_type = $_[10];
	...
	$db_handle = DBI->connect("dbi:mysql:$database:$db_host:$db_port", $db_user, $db_passwd, { RaiseError => 1, AutoCommit => 1 });
	my $sql_fmt = "UPDATE time_record_table SET event_name = UPPER('%s'), user_name = UPPER('%s'), prob_space = UPPER('%s') event_date =  '%s', duration = '%d', event_type = UPPER('%s') WHERE event_name = UPPER('%s'), user_name = UPPER('%s'), prob_space = UPPER('%s') event_date =  '%s', duration = '%d', event_type = UPPER('%s');";
	$sql_buf = sprintf($sql_fmt, $old_event_name, $old_user_name, $old_prob_space, $old_event_date, $old_duration, $old_event_type, $new_event_name, $new_user_name, $new_prob_space, $new_event_date, $new_duration, $new_event_type);
